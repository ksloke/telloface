...
class Tello(object):
    ...
    def __init__(self, port=9000):
        print('Hello TelloPy : '+'192.168.10.1: 8889')
    ...
    def takeoff(self):
        ...
        log.info('set altitude limit 5m')
        pkt = Packet(SET_ALT_LIMIT_CMD)
        pkt.add_byte(0x05)  # 5m
        pkt.add_byte(0x00)
        ...
        while self.state != self.STATE_QUIT:
            ...
            try:
                ...
                if prev_ts is not None and 0.1 < (now - prev_ts).total_seconds():
                    ...
                    (len(data), byte(data[0]), byte(data[1]),(now - prev_ts).total_seconds() * 1000)
                prev_ts = now
...
if __name__ == '__main__':
    print('Tellopy is a library for Tello drones.')
